#> as any
# storage "isc:tmp spell" must contain the spell id

execute if data storage isc:tmp {spell:"magic_missile"    } run return run loot insert ~ ~ ~ loot isc:spells/magic_missile
execute if data storage isc:tmp {spell:"laser"            } run return run loot insert ~ ~ ~ loot isc:spells/laser
execute if data storage isc:tmp {spell:"fireball"         } run return run loot insert ~ ~ ~ loot isc:spells/fireball
execute if data storage isc:tmp {spell:"explosion"        } run return run loot insert ~ ~ ~ loot isc:spells/explosion
execute if data storage isc:tmp {spell:"heal"             } run return run loot insert ~ ~ ~ loot isc:spells/heal
execute if data storage isc:tmp {spell:"teleport"         } run return run loot insert ~ ~ ~ loot isc:spells/teleport
execute if data storage isc:tmp {spell:"summon_vex"       } run return run loot insert ~ ~ ~ loot isc:spells/summon_vex
execute if data storage isc:tmp {spell:"summon_lightning" } run return run loot insert ~ ~ ~ loot isc:spells/summon_lightning
execute if data storage isc:tmp {spell:"clone"            } run return run loot insert ~ ~ ~ loot isc:spells/clone
execute if data storage isc:tmp {spell:"multishot"        } run return run loot insert ~ ~ ~ loot isc:spells/multishot
execute if data storage isc:tmp {spell:"speed_inc"        } run return run loot insert ~ ~ ~ loot isc:spells/speed_inc
execute if data storage isc:tmp {spell:"speed_dec"        } run return run loot insert ~ ~ ~ loot isc:spells/speed_dec
execute if data storage isc:tmp {spell:"range_inc"        } run return run loot insert ~ ~ ~ loot isc:spells/range_inc
execute if data storage isc:tmp {spell:"range_dec"        } run return run loot insert ~ ~ ~ loot isc:spells/range_dec
execute if data storage isc:tmp {spell:"empower"          } run return run loot insert ~ ~ ~ loot isc:spells/empower
execute if data storage isc:tmp {spell:"homing"           } run return run loot insert ~ ~ ~ loot isc:spells/homing
execute if data storage isc:tmp {spell:"random_dir"       } run return run loot insert ~ ~ ~ loot isc:spells/random_dir
execute if data storage isc:tmp {spell:"multicast"        } run return run loot insert ~ ~ ~ loot isc:spells/multicast
execute if data storage isc:tmp {spell:"guide"            } run return run loot insert ~ ~ ~ loot isc:spells/guide
execute if data storage isc:tmp {spell:"summon_bat"       } run return run loot insert ~ ~ ~ loot isc:spells/summon_bat
execute if data storage isc:tmp {spell:"summon_slime"     } run return run loot insert ~ ~ ~ loot isc:spells/summon_slime
execute if data storage isc:tmp {spell:"summon_tnt"       } run return run loot insert ~ ~ ~ loot isc:spells/summon_tnt
execute if data storage isc:tmp {spell:"counterspell"     } run return run loot insert ~ ~ ~ loot isc:spells/counterspell
execute if data storage isc:tmp {spell:"revealing_bolt"   } run return run loot insert ~ ~ ~ loot isc:spells/revealing_bolt
execute if data storage isc:tmp {spell:"heavy_shot"       } run return run loot insert ~ ~ ~ loot isc:spells/heavy_shot
execute if data storage isc:tmp {spell:"long_shot"        } run return run loot insert ~ ~ ~ loot isc:spells/long_shot
execute if data storage isc:tmp {spell:"quick_shot"       } run return run loot insert ~ ~ ~ loot isc:spells/quick_shot
execute if data storage isc:tmp {spell:"ghost"            } run return run loot insert ~ ~ ~ loot isc:spells/ghost
execute if data storage isc:tmp {spell:"double_teleport"  } run return run loot insert ~ ~ ~ loot isc:spells/double_teleport
execute if data storage isc:tmp {spell:"gravity"          } run return run loot insert ~ ~ ~ loot isc:spells/gravity
execute if data storage isc:tmp {spell:"anti_gravity"     } run return run loot insert ~ ~ ~ loot isc:spells/anti_gravity
execute if data storage isc:tmp {spell:"instant"          } run return run loot insert ~ ~ ~ loot isc:spells/instant
execute if data storage isc:tmp {spell:"piercing"         } run return run loot insert ~ ~ ~ loot isc:spells/piercing
execute if data storage isc:tmp {spell:"arcane_reaper"    } run return run loot insert ~ ~ ~ loot isc:spells/arcane_reaper
execute if data storage isc:tmp {spell:"inverse_teleport" } run return run loot insert ~ ~ ~ loot isc:spells/inverse_teleport
execute if data storage isc:tmp {spell:"sheep"            } run return run loot insert ~ ~ ~ loot isc:spells/sheep
execute if data storage isc:tmp {spell:"summon_pufferfish"} run return run loot insert ~ ~ ~ loot isc:spells/summon_pufferfish
