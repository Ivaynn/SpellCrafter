# Triggered when a non-player caster uses a "wand" item
...
# Mana in chat
execute if entity @s[type=minecraft:armor_stand] run tellraw @a[distance=..5] ["",{"score":{"name":"@s","objective":"isc.mana"},"color":"aqua"},{"text":"/","color":"aqua"},{"score":{"name":"@s","objective":"isc.max_mana"},"color":"aqua"}]
