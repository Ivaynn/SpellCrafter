...
execute at @s anchored feet positioned ~ ~-1 ~ facing entity @e[distance=..15,limit=1,sort=nearest,type=!#spellcrafter:untargetable,tag=!spellcrafter.untargetable,tag=!spellcrafter.spectator,predicate=!spellcrafter:match_id] feet store success score $success spellcrafter.tmp run tp @s ~ ~1 ~ ~ ~
