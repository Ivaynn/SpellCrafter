...
execute store result score $spell.homing spellcrafter.tmp run tp @s ~ ~-0.75 ~ facing entity @e[limit=1,distance=0.1..30,sort=nearest,type=!#spellcrafter:untargetable,tag=!spellcrafter.untargetable,tag=!spellcrafter.spectator,predicate=!spellcrafter:match_id,predicate=!spellcrafter:homing_immune] feet
